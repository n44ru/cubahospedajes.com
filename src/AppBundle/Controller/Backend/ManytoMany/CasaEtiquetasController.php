...
class CasaEtiquetasController extends Controller
{
    ...
    public function indexAction(Request $request, $id)
    {
        ...
        if ($request->request->count() >= 1) {
            $query = $em->createQuery('DELETE FROM AppBundle:CasaEtiqueta p WHERE p.casaid ='.$id);
            $query->getResult();
            //
            $x = 0;
        ...
        return $this->render('backend/casas/etiquetas/ver.html.twig', array('etiquetas' => $etiquetas, 'ce' => $casa_etiquetas, 'casa' => $casa_object, 'alertas'=> $alertas));
    }
    /**
